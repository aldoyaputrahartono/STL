...
int main(){
    ...
    for(int i=1;i<=5;i++) map_int_int[i]=i*5; // (key,value) = (1,5) , (2,10) , (3,15) , (4,20) , (5,25)

    map_int_string[1] = "satu";
    map_int_string[2] = "dua";
    map_int_string[3] = "tiga"; // (key,value) = (1,"satu") , (2,"dua") , (3,"tiga")

    for(it=map_int_string.begin();it!=map_int_string.end();it++){
    ...
    for(int i=1;i<=7;i++){
        if(map_int_int.count(i)) cout<<"elemen dengan key "<<i<<" ada di dalam map"<<endl;
        else cout<<"elemen dengan key "<<i<<" tidak ada di dalam map"<<endl;
    }
    return 0;
}
